...
function updateExpression(button) {
    ...
    if (!isNaN(value)) {
      // Jei paskutinis buvo skaičius, užtikriname, kad galėsime įrašyti kitą skaičių
      if (lastValueDigit) {
        expression += value;
      } else {
        expression += value;
      ...
      lastValueDigit = true; // Paskutinis įrašytas simbolis yra skaičius
    } else if (value === "+" || value === "-" || value === "*") {
      // Jei paskutinis simbolis nebuvo skaičius, operatorius nebus įrašytas
      if (!lastValueDigit) {
        messsageEl.textContent =
          "Invalid input! Please enter a number before an operator.";
        return;
      } else {
        expression += value;
        lastValueDigit = false; // Po operatoriaus turi sekti skaičius
      }
    }
